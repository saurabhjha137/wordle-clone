...
export function initAuth(onLoginSuccess) {
  ...
  $('reg-password').addEventListener('input', function () {
    $('pw-bar').className = 'pw-bar ' + getStrength(this.value);
  });
  ...
  attachFieldValidation('reg-age',         'err-reg-age',
    v => Validate.age(v === '' ? '' : Number(v)));
  ...
  $('login-form').addEventListener('submit', async e => {
    ...
    if (!ok) {
      ...
      setError('err-login-form',     errs.form     || 'Something went wrong.');
      return;
    }
  ...
  $('register-form').addEventListener('submit', async e => {
    ...
    if (!ok) {
      ...
      setError('err-reg-form',     errs.form     || 'Something went wrong.');
      return;
    }
    saveSession(data.token, data.user);
    onLoginSuccess(data.user);
